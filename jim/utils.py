...
# TODO: how to properly import this?
NAMING = ['M_c', 'q', 's1_z', 's2_z', 'd_L', 't_c', 'phase_c', 'cos_iota', 'psi', 'ra', 'sin_dec']

LABELS = [r'$M_c/M_\odot$', r'$\eta$', r'$\chi_1$', r'$\chi_2$', r'$d_{\rm{L}}/{\rm Mpc}$', r'$t_c$', r'$\phi_c$', r'$\iota$', r'$\psi$', r'$\alpha$', r'$\delta$']

SUPPORTED_WAVEFORMS = ["IMRPhenomD"]
DEFAULT_WAVEFORM = "IMRPhenomD"
...
matplotlib_params = {"axes.grid": True,
        ...
        # "font.serif" : ["Times New Roman"], # keeps giving annoying warning
        "xtick.labelsize": 16,
        "ytick.labelsize": 16,
        "axes.labelsize": 16,
        "legend.fontsize": 16,
...
def str2bool(v):
    """Stupid auxiliary function since argparse with booleans is hard"""
...
def read_prior_from_file(filename: str):
    """
    Reads in a prior from a file specification. Inspired by https://git.ligo.org/lscsoft/bilby/-/blob/d714ddd4078d6c88c411cc1ca7807ba6c98fc5af/bilby/core/prior/dict.py#L127
...
def plot_chains(chains: jnp.array, 
                ...
                convert_iota_dec: bool = False):
    
    chains = np.array(chains)
    print("np.shape(chains)")
    print(np.shape(chains))
    n_dim = jnp.shape(chains)[-1]
...
def load_true_params_from_config(outdir: str, convert_iota_dec: bool = False):
...
def get_parser(**kwargs):
    ...
    parser.add_argument(
        ...
        type=int,
        default=13245,
        help="Seed used to generate the noise in the detector.",
    )
    parser.add_argument(
        "--sampling-seed",
        type=int,
        default=42,
        help="Seed used during sampling.",
    )
    parser.add_argument(
        "--outdir",
        type=str,
    ...
    # TODO: modify so that PSDs and CSDs can differ
    parser.add_argument(
        "--psd-file-H1",
        type=str,
        default="psd.txt",
        help=f"Filename of the PSD to be used for H1.",
    )
    parser.add_argument(
        "--psd-file-L1",
        type=str,
        default="psd.txt",
        help=f"Filename of the PSD to be used for L1.",
    )
    parser.add_argument(
        "--psd-file-V1",
        type=str,
        default="psd_virgo.txt",
        help=f"Filename of the PSD to be used for V1.",
    )
    parser.add_argument(
        "--waveform-approximant",
        type=str,
        default=f"{DEFAULT_WAVEFORM}",
        help=f"Which waveform approximant to use. Supported waveforms are: {SUPPORTED_WAVEFORMS}. Default: {DEFAULT_WAVEFORM}",
    )
    # TODO: check for the overlapping signals
    parser.add_argument(
        "--use-scheduler",
        type=str2bool, 
        nargs='?',
    ...
    parser.add_argument(
        ...
        const=True, 
        default=False,
        help="Whether to use relative binning or not.")
    parser.add_argument(
    ...
    ######################################
    ### JIM AND FLOWMC HYPERPARAMETERS ###
    ######################################
    parser.add_argument(
        "--n-loop-training",
        type=int,
